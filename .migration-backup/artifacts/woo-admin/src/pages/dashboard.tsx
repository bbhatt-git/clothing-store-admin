import { useGetDashboardStats, useGetSalesChart, useGetRecentOrders, useGetLowStockProducts } from "@workspace/api-client-react"
...
export default function Dashboard() {
  ...
  const { data: salesData } = useGetSalesChart({ period: "30days" })
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground">Store overview and performance metrics.</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{stats?.totalSales || "$0.00"}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
      ...
      <Card className="col-span-4">
        <CardHeader>
          <CardTitle>Sales Overview (30 Days)</CardTitle>
        </CardHeader>
              ...
              <TableBody>
                {recentOrders?.slice(0, 5).map((order) => (
                  <TableRow key={order.id}>
                    ...
                    <TableCell>
                      <Badge variant={order.status === 'completed' ? 'default' : 'secondary'} className="rounded-none">
                        {order.status}
                      </Badge>
                    </TableCell>
              ...
              <TableBody>
                {lowStock?.slice(0, 5).map((product) => (
                  <TableRow key={product.id}>
                    ...
                    <TableCell className="text-right text-destructive">{product.stock_quantity}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
