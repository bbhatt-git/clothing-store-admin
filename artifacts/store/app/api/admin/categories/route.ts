...
export async function GET(req: NextRequest) {
  ...
  const res = await wooFetch(`products/categories?${buildWooQuery(params)}`);
  const data = await res.json();
  const total = parseInt(res.headers.get("X-WP-Total") || "0", 10);
  const totalPages = parseInt(res.headers.get("X-WP-TotalPages") || "1", 10);
  if (!res.ok) return NextResponse.json({ error: "Error" }, { status: 500 });
  return NextResponse.json({ categories: data, total, totalPages });
}
...
export async function POST(req: NextRequest) {
  ...
  const res = await wooFetch("products/categories", { method: "POST", body: JSON.stringify(body) });
  const data = await res.json();
