      ...
      <div className="max-w-[1560px] mx-auto px-6 md:px-10 py-12 md:py-20">
        <div className="grid grid-cols-1 items-center gap-8 md:gap-12 lg:grid-cols-2">
          <header className="flex flex-col gap-8">
            ...
            <p className="text-[#121212]/60 max-w-lg text-base sm:text-lg leading-relaxed">
            ...
            <div className="relative max-w-md">
              <div className="cursor-pointer" onClick={() => setIsSearchOpen(true)}>
                <input
                  ...
                  className="h-14 w-full rounded-sm border border-stone-300 bg-white pe-24 pl-12 text-sm text-[#121212] placeholder:text-stone-400 focus:border-zinc-900 focus:ring-2 focus:ring-zinc-900 focus:ring-offset-2 outline-none transition-all cursor-pointer"
                  readOnly
                />
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3">
              <Link href="/shop" className="inline-flex h-10 cursor-pointer items-center justify-center rounded-sm bg-[#121212] px-6 text-xs font-bold uppercase tracking-widest text-white hover:bg-[#FE5733] transition-colors whitespace-nowrap w-full sm:w-auto">
                Shop Now
                <svg className="ml-2 size-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg>
              </Link>
              <Link href="/shop" className="inline-flex h-10 cursor-pointer items-center justify-center rounded-sm border border-[#121212] px-6 text-xs font-bold uppercase tracking-widest text-[#121212] hover:bg-[#121212] hover:text-white transition-colors whitespace-nowrap w-full sm:w-auto">
                <svg className="mr-2 size-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>
                View Catalog
              </Link>
            </div>
          </header>
          ...
          <div className="flex flex-col gap-4">
              ...
              <>
                <div className="relative h-[320px] sm:h-[420px] md:h-[500px] w-full overflow-hidden rounded-lg shadow-sm bg-white">
                  <div className={`relative h-full transition-all duration-700 ease-out ${isAnimating ? 'opacity-0 scale-95 blur-sm' : 'opacity-100 scale-100 blur-0'}`}>
                    <img
                      src={currentProduct.images?.[0] || 'https://images.pexels.com/photos/17474229/pexels-photo-17474229.jpeg?auto=compress&cs=tinysrgb&w=1200'}
                    ...
                    <div className={`absolute inset-0 flex flex-col justify-end p-8 transition-all duration-700 delay-100 ${isAnimating ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}`}>
                      <div className="relative z-10 max-w-md flex flex-col gap-4">
                        ...
                        <h2 className="text-4xl font-bold uppercase tracking-tight text-white">{currentProduct.name}</h2>
                        <div className="flex items-center gap-4 pt-2">
                          <Link href={`/shop/${currentProduct.slug}`} className="group inline-flex h-12 cursor-pointer items-center justify-center rounded-sm bg-[#FE5733] px-8 text-sm font-bold uppercase tracking-widest text-white transition-all hover:bg-white hover:text-[#121212] hover:shadow-lg whitespace-nowrap">
                            View Product
                            <svg className="ml-2 size-4 transition-transform group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg>
                          </Link>
                        </div>
                      </div>
                    </div>
                ...
                {featuredProducts.length > 1 && (
                  <div className="flex justify-center gap-2">
                    {featuredProducts.map((_, i) => (
                      <button key={i} onClick={() => setCurrentSlide(i)} className={`w-2 h-2 rounded-full transition-all ${i === currentSlide ? 'bg-[#FE5733] w-6' : 'bg-stone-300'}`} />
                    ))}
                  </div>
                )}
              </>
            ...
            {!currentProduct && (
              <div className="h-[400px] bg-stone-100 rounded-lg flex items-center justify-center">
                <p className="text-stone-400 text-sm">No featured products yet</p>
              </div>
            )}
          </div>
        </div>
